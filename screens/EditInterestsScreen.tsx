...
type GroupScreenRouteProp = RouteProp<RootStackParamList, 'InterestList'>;

type GroupScreenNavigationProp = StackNavigationProp<RootStackParamList,'InterestList'>;
type Props = {
...
const NewEventComponent = ({route,navigation}:Props) => {
  ...
  const createEvent = async () => {
    ...
    if (newEventState.interestsarray.length > 0) {
     // newEvent.skills = [];

      // Value should be a ;-delimited list of email addresses
      // NOTE: The app does no validation of this
     // const interests_list = newEventState.interests.split(';')

      newEventState.interestsarray.forEach((interest:any) => {
        newEvent.interests!.push(interest);
      });
    }
  ...
  const addItem = async () => {
    ...
    newEventState.interestsarray.push(newEventState.interests)
    newEventState.updateValue(false, 'isCreating');
  ...
  const removeItem =(item:any) => {
    ...
    const filteredItems = newEventState.interestsarray.filter(item1 => item1 !== valueToRemove)
    newEventState.interestsarray=filteredItems;
    console.log('interestarray',newEventState.interestsarray)
    newEventState.updateValue(false, 'isCreating');
  }
...
<Modal visible={newEventState.isCreating}>
      ...
      <View style={styles.searchbar}>
        ...
        {/* <Image source={require('../images/left-arrow.png')}
                ...
                <Text style={{fontFamily:'Segoe UI',flex:1,marginTop:2, padding: 10,color:'#000',fontSize:22,justifyContent:'flex-start',fontWeight:'600'}}>Interests</Text>
   ...
   >
    ...
    <ScrollView style={styles.container}>  
        <View style={styles.formField}>
          ...
          <View style={styles.profileView}>{newEventState.interestsarray.map((item, key)=>(
      ...
      <MaterialCommunityIcons name="circle" color={'#4e4e4e'} size={10}style={{marginTop:5,marginHorizontal: 10}} />
        ...
        <TextInput style={styles.textInput}
            ...
            onChangeText={(text) => newEventState.updateValue(text, 'interests')} />
...
export default class NewEventScreen extends React.Component<Props>  {
  ...
  async componentDidMount() {
    const{interests}=this.props.route.params;
  ...
  render() {
    return (
      <NewEventState.Provider value={this.state}>
         <RootStack.Navigator initialRouteName="Interests">
  <RootStack.Screen name="Interests" component={NewEventComponent}
  options={{
    headerShown: false,
