...
type GroupScreenRouteProp = RouteProp<RootStackParamList, 'PastProjects'>;
...
type NewEventState = {
  isCreating: boolean;
  pastProjects: string;
  pastprojectsarray:Array<String>;
...
const NewEventComponent = ({route,navigation}:Props) => {
  ...
  const createEvent = async () => {
    ...
    if (newEventState.pastprojectsarray.length > 0) {
     // newEvent.skills = [];

      // Value should be a ;-delimited list of email addresses
      // NOTE: The app does no validation of this
     // const pastProjects_list = newEventState.pastProjects.split(';')

     newEventState.pastprojectsarray.forEach((pastProject:any) => {
        newEvent.pastProjects!.push(pastProject);
      });
    }
  ...
  const addItem = async () => {
    ...
    newEventState.pastprojectsarray.push(newEventState.pastProjects)
    newEventState.updateValue(false, 'isCreating');
  ...
  const removeItem =(item:any) => {
    ...
    const filteredItems = newEventState.pastprojectsarray.filter(item1 => item1 !== valueToRemove)
    newEventState.pastprojectsarray=filteredItems;
    newEventState.updateValue(false, 'isCreating');
  }
  return (
    ...
    <Modal visible={newEventState.isCreating}>
      ...
      <View style={styles.searchbar}>
        ...
        {/* <Image source={require('../images/left-arrow.png')}
                ...
                <Text style={{fontFamily:'Segoe UI',flex:1,marginTop:2, padding: 10,color:'#000',fontSize:22,justifyContent:'flex-start',fontWeight:'600'}}>Projects</Text>
   ...
   >
    ...
    <ScrollView style={styles.container}>  
        <View style={styles.formField}>
          ...
          <View style={styles.profileView}>{newEventState.pastprojectsarray.map((item, key)=>(
      ...
      <MaterialCommunityIcons name="circle" color={'#4e4e4e'} size={10}style={{marginTop:5,marginHorizontal: 10}} />
        ...
        <TextInput style={styles.textInput}
            ...
            onChangeText={(text) => newEventState.updateValue(text, 'pastProjects')}
             />
       <TouchableOpacity
     onPress={addItem}
     style={[
...
export default class NewEventScreen extends React.Component<Props>  {
  ...
  async componentDidMount() {
    const{projects}=this.props.route.params;
...
<RootStack.Navigator initialRouteName="Projects">
  <RootStack.Screen name="Projects" component={NewEventComponent}
  options={{
    headerShown: false,
  }}
  />
